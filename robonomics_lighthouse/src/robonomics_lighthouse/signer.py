...
class Signer:
    def __init__(self):
        ...
        def sign_ask(msg):
            ...
            signed_message = self.web3.eth.account.signHash(askhash(msg), private_key=self.__eth_private_key)
            msg.signature = signed_message.signature
        ...
        def sign_bid(msg):
            msg.nonce = os.urandom(32)
            signed_hash = self.web3.eth.account.signHash(bidhash(msg), private_key=self.__eth_private_key)
        ...
        def sign_res(msg):
            signed_hash = self.web3.eth.account.signHash(reshash(msg), private_key=self.__eth_private_key)
            msg.signature = signed_hash.signature
